<template>
  <div class="app-container ">
    ...
    <div class="btn_box topicStep_box">
      ...
      <router-link :to="{path:'/competition/penetration/index'}">
        <el-button style="margin-left: 10px;" type="primary">完成</el-button>
      </router-link>

      <router-link :to="{path:'/competition/penetration/operation',query:{id:id}}"><el-button style="margin-left: 10px;" type="warning">竞赛运维</el-button></router-link>
    </div>
...
export default {
  ...
  methods: {
    ...
    addInternet() { // 添加网络
      this.$router.push({
        path: '/competition/penetration/editMatch/addInetnet', query: { id: this.id, step: 3 }
      },)
    },
    editInternet(i_id) { // 添加网络
      this.$router.push({
        path: '/competition/penetration/editMatch/addInetnet', query: { id: this.id, internet_id: i_id, step: 3 }
      },)
    },
    removeSub() { // 删除网络
