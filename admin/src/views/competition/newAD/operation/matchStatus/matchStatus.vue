  ...
  <div class="app-container">
    <el-form label-width="130px">
      ...
      <el-form-item v-if="event_mode==1" label="VPN部署">
        <el-button v-if="matchInfo.vpn_deploy_status===1" type="success" size="mini" @click="vnpDeploy">部署</el-button>
        <el-button v-else type="danger" size="mini" @click="removeDeploy">回收</el-button>
        ...
        <el-button v-if="matchInfo.vpn_deploy_status===2" type="primary" size="mini" @click="vpnCaDeploy">申请证书</el-button>
      ...
      <el-form-item label="回合结束时间">{{ matchInfo.round_end_time }}</el-form-item>

      <el-form-item label="当前服务器时间">{{ matchInfo.time }}</el-form-item>

      <!--<el-form-item label="vpn开关">-->
      <!--<el-switch-->
      <!--v-model="matchInfo.vpn_status"-->
      <!--:active-value="1"-->
      <!--:inactive-value="0"-->
      <!--@change="vpnfun"-->
      <!--/>-->
      ...
      <el-form-item label="比赛控制">
        <el-button
          ...
          type="success"
          size="mini"
          @click="handleSwitch(0,'开始比赛')"
        >开始比赛</el-button>
        <el-button
          v-show="matchInfo.status==2"
          type="success"
          size="mini"
          @click="handleSwitch(2,'恢复比赛')"
        >恢复比赛</el-button>
        <el-button
          v-show="matchInfo.status==3"
          type="warning"
          size="mini"
          @click="handleSwitch(1,'暂停比赛')"
        >暂停比赛</el-button>
        <el-button
          v-show="matchInfo.status!=1&&matchInfo.status!=4&&matchInfo.status!=0"
          type="danger"
          size="mini"
          @click="handleSwitch(3,'结束比赛')"
        >结束比赛</el-button>
      </el-form-item>

      <el-form-item label="数据清理">
        <el-button type="danger" size="mini" @click="handleSwitch(4,'初始化用户数据')">初始化用户数据</el-button>
        <el-button type="danger" size="mini" @click="initVirtual">初始化虚拟数据</el-button>
      </el-form-item>
...
import {
  ...
  initSence,
  initVirtual,
  fetchDefenseInfo, pushflag
} from '@/api/match/newAD'
import { vnpDeploy, vpnCaDeploy, removeDeploy, controlJoin } from '@/api/match/vpn'
