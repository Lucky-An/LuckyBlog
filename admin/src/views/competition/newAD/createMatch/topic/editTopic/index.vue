<template>
  <div class="app-container create_newAD_task">
    ...
    >
      ...
      <el-form-item label="题目端口" prop="port">
        <el-input-number
      ...
      <el-form-item label="实例模板">
        ...
        >
          <!-- <el-option
            ...
            :key="item.uuid"
            :value="item"
            :label="`${item.name} / ${item.msg.vcpus}核 / ${
              item.msg.memory / 1024 / 1024 / 1024
            }MB / ${item.msg.disk / 1024 / 1024 / 1024}GB`"
          /> -->
          <el-option
      ...
      <el-form-item label="上传题目二进制文件">
        <UploadFile :file.sync="ruleForm.attach" :file-size="1 * 1024 * 1024" />
        （大小不超过1M）
      </el-form-item>

      <el-form-item label="checker 脚本">
        <UploadFile
          :file.sync="ruleForm.checker_script"
          :file-type="['py']"
        />（请上传py格式文件，大小不超过 10 M）
      </el-form-item>

      <el-form-item label="pushflag 脚本">
        <UploadFile
          :file.sync="ruleForm.pushflag_script"
          :file-type="['sh']"
        />（请上传sh格式文件，大小不超过 10 M）
      </el-form-item>
