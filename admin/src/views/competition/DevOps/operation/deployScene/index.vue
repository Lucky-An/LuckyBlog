        ...
        <el-table-column align="center" label="操作">
          <template slot-scope="scope">
            <el-button
              ...
              size="mini"
              type="success"
              @click="startDeloy(scope.row.id)">开始部署
            </el-button>
            <!--<el-button-->
            <!--v-waves-->
            <!--class="filter-item"-->
...
export default {
  ...
  data() {
    return {
      ...
      message: [], // 进度条报错信息。
      matchList: [
        { model: '环境模块', progress: 0, id: 1, status: 'success', statusText: '' },
        { model: '队伍模块', progress: 0, id: 2, status: 'success', statusText: '' },
        { model: '网络模块', progress: 0, id: 3, status: 'success', statusText: '' },
        { model: '题目环境模块', progress: 0, id: 4, status: 'success', statusText: '' }
    ...
    showProgress(name, index) { // 查询进度条
      ...
      this.intervalId = setInterval(() => {
        defenseProgress(params).then(res => {
          ...
          if (data.progress >= 100) { // 如果进度大于100，则终止
            clearInterval(this.intervalId)
          ...
          if (data.fail_task) { // 如果任务启动出现错误，进度条变红
