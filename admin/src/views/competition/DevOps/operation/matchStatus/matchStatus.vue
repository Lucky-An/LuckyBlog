<template>
  <div class="app-container">
    <el-form label-width="130px">
      ...
      <el-form-item label="回合时间">
        {{ matchInfo.round_time }}&nbsp;s
      </el-form-item>
      ...
      <el-form-item label="数据清理">
        ...
        <el-button
          type="danger"
          size="mini"
          @click="handleInitVirData"
        >初始化虚拟数据
        </el-button
        >
      </el-form-item>
      <el-form-item label="checker文件推送">
        <el-button
          type="success"
          size="mini"
          @click="pushflag"
  ...
  methods: {
    ...
    handleInitVirData() {
      this.$confirm('确定要初始化虚拟数据吗？', '提示', {
        ...
        .then(() => {
          initVirData(this.params).then((res) => {
            if (res.data.success) {
    ...
    vpnfun() {
      controlJoin(this.$route.query.id, this.matchInfo.vpn_status).then(
        (res) => {
          if (res.data.success) {
            this.getList()
          }
    ...
    // 切换vpn展示状态
    changeVpn(val) {
      changeVpn(this.params, { action: val }).then((res) => {
        if (res.data.success) {
          this.$message.success('操作成功')
        }
      })
