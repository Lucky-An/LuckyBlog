<template>
  <div class="app-container competition_box">
    ...
    <div class="filter-container">
      ...
      <!--<el-input-->
      <!--v-model.trim="listQuery.sponsor_zh"-->
    ...
    <el-table
      ...
      <el-table-column
        ...
        label="公开/隐藏">
        <template slot-scope="scope">
          <span>{{ scope.row.public==1?'公开':'隐藏' }}</span>
        </template>
      </el-table-column>
      <el-table-column
        align="center"
...
<script>
import { fetchDefenseList, deleteDefenseMatch, publicDefenseMatch, fetchDefenseInfo } from '@/api/match/defense'
...
export default {
  ...
  methods: {
    ...
    addMatch() { // 新增竞赛
      this.$router.push({
        path: '/competition/defense/createMatch'
      })
    },
    editMatch(id) { // 编辑竞赛
      this.$router.push({
    ...
    accredit(id, mode) { // 授权管理
      this.$router.push({
        path: '/competition/defense/accredit',
        query: {
          id: id,
          mode: mode
        }
