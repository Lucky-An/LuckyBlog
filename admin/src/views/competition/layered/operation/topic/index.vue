    ...
    <div class="filter-container">
      ...
      <el-table
        ...
        <el-table-column
          ...
          label="flag类型">
          <template slot-scope="scope">
            <span v-if="scope.row.is_share==0">静态</span>
            <span v-if="scope.row.is_share==1">动态</span>
            <span v-if="scope.row.is_share==2">随机</span>
          </template>
        </el-table-column>
        <el-table-column
          align="center"
    ...
    <el-dialog :visible.sync="addDlg" class="addsub" title="加入题目" width="900px">
      <div style="margin-bottom: 20px;">
        ...
        <el-select v-model="topicQuery.tag" class="filter-item" size="mini" placeholder="比赛公告">
          ...
          <el-option v-for="item,index in subTag" :key="ind" :label="item.cn_name" :value="item.id"/>
        </el-select>
...
export default {
  ...
  created() {
    this.is_share = getStorage('match_info', 'object').is_share
    this.getList()
    this.getType()
  },
  beforeDestroy() {
  ...
  methods: {
    ...
    addTopic() { // 新增赛题
      this.$router.push({
        path: '/competition/layered/operation/addTopic',
        query: { id: this.$route.query.id }
      })
    },
    editTopic(id) { // 编辑题目
      this.$router.push({
        path: '/competition/layered/operation/editTopic',
        query: {
