<template>
  <div class="app-container create_topic">
    <el-form ref="ruleForm" :model="ruleForm" :rules="rules" label-width="200px" class="demo-ruleForm">
      ...
      <el-form-item label="积分方式" placeholder="请填写题目名称">
        <span>{{ mathMode.integral_mode==2?'动态计分':'累加计分' }}</span>
      </el-form-item>
      <el-form-item v-if="mathMode.integral_mode==1" label="题目分值" prop="score" placeholder="请填写题目分值">
...
export default {
  ...
  methods: {
    goback() {
      this.$router.push({
        path: '/competition/layered/operation/index',
        query: { id: this.$route.query.id, activeName: 'topic' }
      })
    },
    getMode() {
    ...
    onSubmit() {
      if (this.flagAry[0].flag === '' && this.ruleForm.is_dynamic_flag != 1) {
        this.$message.error('请至少填写一个flag')
        return
      }
      const AllImgExt = '.zip'
