<template>
  <div class="app-container">
    <div class="filter-container">
      <el-select v-model="noiceType" size="mini" placeholder="竞赛公告" @change="handleType">
    ...
    <div class="filter-container">
      <el-button type="success" icon="el-icon-circle-plus" size="mini" @click="showDlg"> 新增</el-button>
      <el-button type="danger" icon="el-icon-circle-close" size="mini" @click="romoveNoice"> 删除</el-button>
      <el-button type="success" icon="el-icon-upload" size="mini" @click="topJeopardyNoice(1)"> 置顶</el-button>
      <el-button type="danger" icon="el-icon-upload" size="mini" @click="topJeopardyNoice(0)"> 取消置顶</el-button>

      <div style="float: right;">
        <el-input
          v-model.trim="listQuery.search"
          :placeholder="noiceType===2?'题目提示':'竞赛公告'"
          style="width: 150px;"
          size="mini"
          class="filter-item"/>
...
export default {
  ...
  methods: {
    ...
    submitNotice() { // 发布、编辑 公告
      if (this.noticeId) {
        this.noiceType === 1 ? this.updateJeopardyNoice() : this.updateJeopardyTIP()
      } else {
        this.noiceType === 1 ? this.addJeopardyNoice() : this.addJeopardyTIP()
      }
    },
    updateJeopardyNoice() { // 提交公告编辑
      updateJeopardyNoice(this.noticeId, this.addQuery).then(res => {
        if (res.data.success) {
    ...
    updateJeopardyTIP() { // 提交提示编辑
      updateJeopardyTIP(this.noticeId, this.addQuery).then(res => {
        if (res.data.success) {
          this.$message.success('修改成功')
          this.getList()
          this.addDlg = false
    ...
    getTipList() { // 获取提示列表
      ...
      data.task_id = ''
      fetchJeopardyTipList(data).then(res => {
        this.matchList = res.data.rows
        this.total = res.data.total
      })
    },
