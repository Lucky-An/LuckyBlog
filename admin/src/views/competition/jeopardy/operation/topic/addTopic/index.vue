<template>
  <div class="app-container create_topic jeopardy_task">
    <el-form
      ref="ruleForm"
      :model="ruleForm"
      :rules="rules"
    ...
    >
      ...
      <!-- 题目标签 -->
      <el-form-item label="题目标签" prop="tag_id">
        <el-select
          :disabled="task_id ? true : false"
      ...
      <el-form-item label="Flag类型" prop="is_dynamic_flag">
        <el-select
          :disabled="task_id ? true : false"
        ...
        >
          ...
          <el-option :value="1" label="动态Flag" />
          <el-option :value="2" label="随机Flag" />
        </el-select>
      </el-form-item>
      <!-- 镜像类型选择 -->
      <template v-if="ruleForm.is_dynamic_flag != '2'">
        <el-form-item label="镜像类型" prop="image_type">
          <el-select
            :disabled="task_id ? true : false"
          ...
          >
            ...
            <el-option value="docker-compose" label="docker-compose" />
          </el-select>
        </el-form-item>
      </template>
      ...
      <!-- 端口 -->
      <template
        v-if="ruleForm.is_dynamic_flag == '0' && ruleForm.image_type != 'kvm'"
      >
      ...
      <template
        ...
        "
      >
        <el-form-item label="镜像选择" class="red_start">
          <el-button
            :disabled="task_id ? true : false"
            type="primary"
            size="mini"
      ...
      <div v-if="ruleForm.is_dynamic_flag == 0">
        ...
        <el-form-item label="题目Flag" class="red_start">
          <el-input
      ...
      <div v-if="ruleForm.is_dynamic_flag == 1">
        ...
        <el-form-item label="动态flag脚本文件" prop="is_dynamic_flag">
          <!-- <UploadFile
          ...
          <span v-else> {{ ruleForm.push_flag_script }} </span>
        </el-form-item>
      ...
      <div v-if="ruleForm.is_dynamic_flag == 2">
        <el-form-item label="题目附件" prop="is_dynamic_flag" required>
          <div>
            <el-button type="success" icon="el-icon-plus" @click="addEnc" />
          ...
          <div class="randomBox">
            ...
            <div class="flagBox">
              ...
              >
                flag:
                <el-input :disabled="task_id ? true : false" v-model.trim="ruleForm.flag[index]" width="300px" />
                <el-button
...
export default {
  ...
  methods: {
    // ! 分页切换
    getPageList(params) {
      this.listSelect.limit = params.limit
      this.listSelect.offset = params.page
      this.getMirrorList()
    },
    // 弹窗中点击确认的事件
    confirmSelect() {
    ...
    goBack() {
      // history.back();
      this.$router.push({
        path: '/competition/jeopardy/operation/index',
        query: {
    ...
    subInfo() {
      ...
      showJeopardyEditInfo(this.task_id).then((res) => {
        ...
        // 锚点
        if (res.data.image_uuid && res.data.image_name) {
          this.selectMirrorList = [
            { name: res.data.image_name, uuid: res.data.image_uuid }
          ]
        }
    ...
    onSubmit() {
      ...
      if (this.task_id) {
        ...
        createJeopardyTopic(fm).then((res) => {
          if (res.data.success) {
            this.$message.success('操作成功')
            this.$router.push({
              path: '/itemBank/jeopardy/index',
              query: { id: res.data.event_id, activeName: 'topic' }
            })
          }
        })
      }
...
<style lang="scss">
.jeopardy_task .el-table__header-wrapper {
  th:first-child {
    div.cell {
      display: none;
    }
