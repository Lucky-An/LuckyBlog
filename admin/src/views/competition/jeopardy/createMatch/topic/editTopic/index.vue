    ...
    >
      ...
      >
        <el-form-item label="配置文件">
          <el-col :span="2">
            <el-button type="primary" @click="getConfigList">选择 </el-button>
          </el-col>
      ...
      <!-- 端口 -->
      <template
        v-if="ruleForm.is_dynamic_flag != '2' && ruleForm.image_type != 'kvm'"
      >
        <el-form-item label="题目端口" prop="source_port">
          <el-input-number
...
import { fetchTag } from '@/api/itemBank/jeopardy'
  ...
  data() {
    var checkScore = (rule, value, callback) => {
      ...
      if (isNaN(value)) {
        ...
        if (value > 1000000) {
          callback(new Error('分值范围为0-1000000'))
        } else {
          callback()
        }
      }
    ...
    return {
      ...
      rules: {
        ...
        flag: [{ required: true, trigger: 'change' }],
        category_id: [
          {
            type: 'number',
            required: true,
      ...
      process: null, // 比赛进程 1未开始 2已结束 0进行中
  ...
  methods: {
    ...
    getProcess() {
      matchJeopardyInfo({ evt_hash: this.$route.query.hash }).then((info) => {
        this.process = info.data.process
      })
    },
    goBack() {
      this.$router.push({
    ...
    onSubmit() {
      ...
      if (this.task_id) {
        editJeopardyTopic(this.task_id, fm)
          .then((res) => {
            if (res.data.success) {
              this.$router.push({
                path: '/competition/jeopardy/editMatch/editMatch',
