        ...
        loginByUsername(userInfo).then(response => {
          ...
          if (data.success) {
            if (data.role == 1) {
              ...
              setToken('admin')
            } else if (data.role == 3) { // 裁判员
              setToken('referee')
              commit('SET_TOKEN', 'referee')
            } else if (data.role == 4) { // 教员
              setToken('teacher')
              commit('SET_TOKEN', 'teacher')
            }
          }
          resolve()
        }).catch(error => {
    ...
    GetUserInfo({ commit, state }) {
      return new Promise((resolve, reject) => {
        ...
        getConfig().then(response => {
          const info = userMap[response.data.role_num]
