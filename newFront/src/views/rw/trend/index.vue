<template>
  <div>
    <div v-show="error" class="guide_wrap" style="padding:15px 0 0 0;">
      <div v-show="show_trend" id="trend_chart" />
      <div v-show="!show_trend" style="font-size:22px;color:white;margin-top:30px;">{{ $t('competition.notBegun') }}</div>
    </div>
    <div v-show="!error" style="margin-top:100px;text-align: center">
      <img src="@/assets/common/kong.png" alt="">
    </div>
  </div>
</template>

<script>
import { trends } from '@/api/rw'
  ...
  methods: {
    ...
    initChart() {
      trends({
        ...
        const rank_num_list = [this.$t('trend.dialog.first'), this.$t('trend.dialog.second'), this.$t('trend.dialog.third')]
        this.error = true
        ...
        objs.forEach((obj, i) => {
          ...
          if (maxTime == endTime && lastSubmitTime && lastSubmitTime != endTime) {
            data.push({
              value: [maxTime, sumScore]
            })
          }
