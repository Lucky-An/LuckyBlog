/* File Info
 * Author:      LuckyAn
 * CreateTime:  2020/9/28 上午11:14:12
 * LastEditor:  LuckyAn
 * ModifyTime:  2020/10/13 下午5:27:42
 * Description:
*/
<template>
  <div>
    ...
    >
      ...
      <div>
        ...
        >
          <div
            class="clearfix"
            style="border: 1px solid hsla(0,0%,100%,.1);display:flex;justify-content:flex-start"
          >
            <div
              v-if="show_randar"
              id="trend_chart"
...
import { ranks, rankTasks, tasks_detail } from '@/api/jad'
...
export default {
  ...
  created() {
    ...
    this.getTableData()
    // this.getDetail()
    console.log(this.match, 'matchmatch')
    this.rank_timer = setInterval(() => {
      if (this.match.remain_seconds > 0) {
        this.getDetail()
      } else {
  ...
  methods: {
    ...
    tableClick(row) {
      console.log(this.not_click_row)
