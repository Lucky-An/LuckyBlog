    ...
    <div class="up_box clearfix">
      ...
      <div class="upload fl">
        <UploadFile
          ...
          class="UploadFile"
        />
        <el-button size="mini" type="primary" @click="uploadWp">{{ $t('writeup.upload') }}</el-button>
      </div>
      <!--上传要求-->
      <span class="fr format">{{ $t('writeup.wp_format') }}</span>
    </div>

    <wpInfo :list="list" @removeWp="removeWp" />
  </div>
</template>
...
import { getWP, removeWp, uploadWp } from '@/api/newAD'

export default {
  name: 'NoticeList',
  components: { wpInfo, UploadFile },
  data() {
    return {
      list: [], // 公告列表
      data: {
  ...
  methods: {
    // 获取wp
    getWP() {
      getWP({ event_id: this.$route.query.id, evt: this.$route.query.hash }).then(res => {
        this.list = res.data.data
      })
    },
    // 删除wp
    removeWp() {
      removeWp({ event_id: this.data.event_id }, this.$route.query.hash).then(res => {
        this.getWP()
        this.$notify.success({
