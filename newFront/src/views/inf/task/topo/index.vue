              ...
              >
                ...
                >
                  <img
                    :src="bar_list.img[item.type]"
                    :draggable="show_bar"
                    @dragstart="dragRemove($event,i,k)"
                    @dragend="dragendRemoce($event,i)"
                    @click="showTask(item,val.network[0],k)"
                  >
                </el-tooltip>
              </div>
              <div
...
export default {
    ...
    bar_list: {
      ...
      default: () => {
        return { // 左侧拖拽栏列表
          ...
          img: {
            kvm: require('@/assets/match/answered.png')
          }
        }
      }
    }
  ...
  methods: {
    ...
    // 展示题目信息
    showTask(node, container, k) {
      this.$emit('handleTask', node, container, k)
    },
    // 拖拽开始
    drag(e, type) {
      this.isNode = true
    ...
    dropRemove(e) {
      ...
      this.$confirm('确定要删除吗？, 是否继续?', '提示', {
        ...
        this.$emit('handleRemove', type, index)
      }).catch(() => {
        this.$message({
          type: 'info',
          message: '已取消删除'
...
.topo1 {
  background: url("~@/assets/match/bg2.png") no-repeat;
}
.topo2 {
  background: url("~@/assets/match/bg3.png") no-repeat;
}
.topo3 {
  background: url("~@/assets/match/bg4.png") no-repeat;
}
