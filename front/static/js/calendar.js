...
;(function ($, window, document, undefined) {
  ...
  var Calendar = function (elem, options) {
    this.$calendar = elem;
    console.log(this.$calendar,'elem')
    this.defaults = {
      ifSwitch: true,
      hoverDate: false,
      backToday: false
  ...
  Calendar.prototype = {
    ...
    showCalendar: function () { // 输入数据并显示
      ...
      this.$calendarTitle_text.html(year + '/' + dateStr.substr(4, 2)+'<span class="mrg5L">北京</span>');
      ...
      // $.ajax({
      //     url:'/api/personal/daily_checkin_logs',
      //     type:'get',
      //     context:self,
      //     success:function(data){
      //         this.$calendarDate_item.each(function (i) {
      //           for(var j=0;j<data.length;j++){
      //             if(data[j] == $(this).attr('data')){
      //               $(this).addClass('already_sign');
      //             }
      //           }
      //         });
      //     }
    ...
    renderDOM: function () { // 渲染DOM
      ...
      var _titleStr = '<a href="#" class="title"></a>'+
                      '<div class="arrow">'+
                        '<span class="arrow-prev glyphicon el-icon-caret-left"></span>'+
                        '<span class="arrow-next glyphicon el-icon-caret-right"></span>'+
                      '</div>';
