  ...
  <div>
    <div id="gf-topic">
      <div class="subject" style="letter-spacing:2px;">
        <span>{{$t('header.ranking.special')}}</span>
        <span class="subjectStar"></span>
      </div>
    </div>
    <ul class="table_nav clearfix">
      <li v-for="(item,i) in rank_type" :class="{'active':nowType==item.value}" @click="changType(item.value)">{{item.name}}</li>
    </ul>
    ...
    export default {
      ...
      created(){
        this.$http.get('/api/tasks/categorys')
          .then((res)=>{
            this.rank_type = res.data;
            this.changType(this.rank_type[0].value)
          })
          .catch((res)=>{
      ...
      methods:{
        changType(i){
          this.$http.get('/api/rank/scoreboard/',{
            params:{
              category:i
            }
          })
